...
class MoSSimulator:
    ...
    def __init__(
        ...
        if callable(phi):
            ...
            assert len(self._phi) == self.dim_x, (
                f"phi must have length 2^n = {self.dim_x}"
            )

        assert np.all((self._phi >= 0) & (self._phi <= 1)), (
            "phi values must be in [0,1]"
        )
    ...
    def circuit_hadamard_measure(
        self, f: np.ndarray, use_oracle: bool = True
    ) -> QuantumCircuit:
        """
    ...
    def sample_hadamard_measure_statevector(
        self, shots: int, rng: Optional[Generator] = None
    ) -> dict:
        """
    ...
    def sample_hadamard_measure(
        self, shots: int, mode: str = "statevector", **kwargs
    ) -> dict:
        """
